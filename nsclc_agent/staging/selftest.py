...
Single source of truth for both the CLI ``selftest`` command and the pytest
suite. Every 8th→9th migration relevant to the shipped modules is represented,
plus the rejection table: inputs the engine must *refuse* (with the reason it
gives), because refusing ambiguity is as much a part of the contract as the
stage table itself.
"""

from __future__ import annotations

from .tnm import StagingError, stage_from_strings
...
EXPECTATIONS: list[tuple[str, str, str, str]] = [
    ...
    ("T3", "N1", "M0", "IIIA"),
    ("T4", "N1", "M0", "IIIA"),
    # --- N2a single-station (T1N2a → IIB; T3N2a → IIIA) ---
    ("T1a", "N2a", "M0", "IIB"),
    ("T1c", "N2a", "M0", "IIB"),
    ("T2a", "N2a", "M0", "IIIA"),
    ("T2b", "N2a", "M0", "IIIA"),
...
#: Refusal messages double as value-of-information hints, so the substring
#: checks that the *actionable* part survived any edit.
...
def run_selftest() -> tuple[int, int, list[str]]:
    ...
    for t, n, m, needle in REJECTIONS:
        try:
            got = stage_from_strings(t, n, m).stage_group
        except StagingError as exc:
            if needle.lower() not in str(exc).lower():
                failures.append(
                    f"{t}{n}{m or '(empty)'}: refused but message lacked "
                    f"{needle!r}: {exc}"
                )
        except Exception as exc:  # noqa: BLE001
            failures.append(f"{t}{n}{m or '(empty)'}: wrong error type {exc!r}")
        else:
            failures.append(
                f"{t}{n}{m or '(empty)'}: expected refusal, got {got}"
            )
    total = len(EXPECTATIONS) + len(REJECTIONS)
